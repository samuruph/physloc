...
def summarise(metas: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    # built from, which domains they exercise, and how a multi clip's culprits
    ...
    timing = Counter(str(m["violation"].get("culprit_timing") or "shared")
                     for m in invalid)
    ...
    return {
        ...
        "domains": dict(domains), "culprit_timing": dict(timing),
...
def _fig_structure(plt, s, path) -> None:
    """Where the invalid clips sit: level x condition, domain, culprit timing."""
    import numpy as np
    ...
    order = [k for k in ("shared", "sync", "independent")
             if k in (s.get("culprit_timing") or {})]
    bars = axes[2].bar(order, [s["culprit_timing"][k] for k in order],
                       color=[EASY, MODERATE, HARD][:len(order)], width=0.62)
    _bar_labels(axes[2], bars)
    _style(axes[2], "Culprit timing", "", "clips")
