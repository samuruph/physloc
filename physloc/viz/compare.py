...
def _load(rec) -> Dict[str, object]:
    ...
    active = None
    tlp = os.path.join(rec["dir"], "timelines.npz")
    if os.path.exists(tlp):
        with np.load(tlp) as z:
            if "active" in z.files:
                active = np.asarray(z["active"]).astype(bool)
    return {"rgb": ov._rgb(rec["dir"], T),
            "mask": ov._npz(rec["dir"], "violation_mask.npz", "mask"),
            "ref": ov._npz(rec["dir"], "reference_mask.npz", "mask"),
            "active": active}
